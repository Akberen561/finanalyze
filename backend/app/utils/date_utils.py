...
EMBEDDED_TRANSACTION_DATETIME_RE = re.compile(
    r"(?:дата\s+проведения\s+транзакции|transaction\s+date)\s*:\s*"
...
def extract_embedded_transaction_datetime(text_val: Any) -> Optional[dt.datetime]:
    if text_val is None:
        return None
    text = str(text_val)
    match = EMBEDDED_TRANSACTION_DATETIME_RE.search(text)
    if not match:
        return None

    parsed = parse_datetime(f"{match.group('date')} {match.group('time')}")
    if parsed is None:
        return None
    return parsed
