...
from app.utils.date_utils import extract_embedded_transaction_datetime, parse_datetime
        ...
        for ridx, row in df.iterrows():
            ...
            for raw_col, canon in mapped.items():
                ...
                if canon == "operation_ts":
                    ...
                    source_bank = str(context.get("source_bank") or "").strip().lower()
                    if source_bank == "halyk" and d and d.hour == 0 and d.minute == 0 and d.second == 0 and d.microsecond == 0:
                        core["operation_ts"] = None
                    else:
                        core["operation_ts"] = d
            ...
            source_bank = str(context.get("source_bank") or "").strip().lower()
            op_ts = core.get("operation_ts")
            if source_bank == "kaspi" and (
                op_ts is None
                or (op_ts.hour == 0 and op_ts.minute == 0 and op_ts.second == 0 and op_ts.microsecond == 0)
            ):
                embedded_dt = None
                for raw_col in df.columns:
                    embedded_dt = extract_embedded_transaction_datetime(row.get(raw_col))
                    if embedded_dt is not None:
                        break
                if embedded_dt is not None:
                    core["operation_ts"] = embedded_dt
                    core["operation_date"] = embedded_dt.date()
