...
from sqlalchemy import select, func, and_, or_, case, text, cast, Date, delete
from sqlalchemy.ext.asyncio import AsyncSession

from ..config import settings
from ..database import get_db, async_session, async_engine
from ..ingestion.pipeline import IngestionPipeline
from ..models import EsfRecord, Transaction, TransactionExt, TransactionUploadMeta
from ..project_context import ProjectContext, get_current_project_context
from ..utils.date_utils import extract_embedded_transaction_datetime
from ..schemas import (
...
def _is_numeric_operation_type(value: object) -> bool:
    return bool(re.fullmatch(r"\d+", str(value or "").strip()))


def _is_empty_import_transaction(tx: dict) -> bool:
    if tx.get("date") is not None or tx.get("operation_date") is not None:
        return False

    amount_keys = ("amount_tenge", "amount_kzt", "debit", "credit", "amount_currency")
    for key in amount_keys:
        try:
            if float(tx.get(key) or 0) != 0:
                return False
        except Exception:
            pass
    ...
    return not any(str(tx.get(key) or "").strip() for key in text_keys)


def _numeric_operation_type_condition():
    return Transaction.operation_type.op("~")(r"^\s*[0-9]+\s*$")


def _empty_transaction_condition():
    return and_(
        Transaction.date.is_(None),
        Transaction.operation_date.is_(None),
        func.coalesce(Transaction.amount_tenge, 0) == 0,
        func.coalesce(Transaction.debit, 0) == 0,
        func.coalesce(Transaction.credit, 0) == 0,
        func.coalesce(Transaction.amount_currency, 0) == 0,
        func.nullif(func.trim(func.coalesce(Transaction.operation_type, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.sender_name, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.sender_iin_bin, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.sender_account, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.recipient_name, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.recipient_iin_bin, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.recipient_account, "")), "").is_(None),
        func.nullif(func.trim(func.coalesce(Transaction.purpose, "")), "").is_(None),
    )
...
def _tx_signature(tx: dict) -> tuple:
    dt: Optional[datetime] = tx.get("date")
    op_date = tx.get("operation_date")
    return (
        dt.isoformat(timespec="seconds") if dt else str(op_date or ""),
...
def _build_core_transaction_payload(
    ...
    *,
    ...
    op_dt: Optional[datetime] = tx.get("date")
    op_date = tx.get("operation_date") or (op_dt.date() if op_dt else None)
    ...
    sig_source = json.dumps(
        {
            "date": op_dt.isoformat(timespec="seconds") if op_dt else str(op_date or ""),
...
def _make_xlsx_with_single_sheet(sheet_title: str, rows: list[tuple]) -> bytes:
    ...
    for row in rows:
        ws.append(list(row))
    _format_export_worksheet(ws)
    out = BytesIO()
    wb.save(out)
    return out.getvalue()


def _format_export_worksheet(ws, max_width: int = 70) -> None:
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        if any(cell.value is not None and len(str(cell.value)) > 50 for cell in row):
        ...
        if parser_type in {PARSER_HALYK, PARSER_KASPI}:
            ...
            for row in rows[1:]:
                ...
                if op_dt is None:
                    ...
                    continue
                op_date = op_dt.date()
                if parser_type == PARSER_HALYK and _is_midnight(op_dt):
                    op_dt = None
                ...
                purpose_value = _to_str(_get_cell(row, idx_purpose), 1000)
                if parser_type == PARSER_KASPI and op_dt is not None and _is_midnight(op_dt):
                    embedded_dt = extract_embedded_transaction_datetime(purpose_value)
                    if embedded_dt is not None:
                        op_dt = embedded_dt
                        op_date = embedded_dt.date()
...
async def _finalize_ingestion_import(file_id: Optional[str], uploader_email: str, project_id: Optional[str]) -> int:
    ...
    async with async_session() as session:
            ...
            (
                await session.execute(
                    select(Transaction.id).where(
                        ...
                        or_(
                            _numeric_operation_type_condition(),
                            _empty_transaction_condition(),
                        ),
                    )
                )
            ).scalars()
        ...
        if invalid_tx_ids:
            await session.execute(delete(TransactionUploadMeta).where(TransactionUploadMeta.tx_id.in_(invalid_tx_ids)))
            await session.execute(delete(TransactionExt).where(TransactionExt.tx_id.in_(invalid_tx_ids)))
            await session.execute(delete(Transaction).where(Transaction.id.in_(invalid_tx_ids)))
...
def _meaningful_transaction_condition():
    return or_(
        ...
        _non_empty_text_condition(Transaction.operation_type),
    )
...
def _build_transaction_order_by(sort_by: str, sort_dir: str):
    ...
    if key == "currency":
        ...
        if is_desc:
            return [rank_expr.desc(), Transaction.currency.asc(), Transaction.date.desc().nullslast()]
        return [rank_expr.asc(), Transaction.currency.asc(), Transaction.date.desc().nullslast()]
    ...
    primary = expr.desc().nullslast() if is_desc else expr.asc().nullslast()
    return [primary, Transaction.date.desc().nullslast()]
...
@router.get("/import-warnings", response_model=ImportWarningsResponse)
async def list_import_warnings(
    ctx: ProjectContext = Depends(get_current_project_context),
    db: AsyncSession = Depends(get_db),
):
    return ImportWarningsResponse(fraud_warnings=[])
...
async def import_statement(
    ...
    if parser_type == PARSER_TRANSACTIONS_CORE:
        ...
        for idx, tx in enumerate(tx_rows, start=2):
            if _is_numeric_operation_type(tx.get("operation_type")) or _is_empty_import_transaction(tx):
                skipped += 1
                continue
            payloads.append(
